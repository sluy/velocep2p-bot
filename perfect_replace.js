...
// FIX: Insert missing </div> for <div className="bg-indigo-950/20 ..."> right before <div className="bg-slate-900 border-l border-slate-700/50 p-6 flex flex-col pt-12">
modalFile = modalFile.replace(/<div className="bg-slate-900 border-l border-slate-700\/50 p-6 flex flex-col pt-12">/, '</div>\n\n                        <div className="bg-slate-900 border-l border-slate-700/50 p-6 flex flex-col pt-12">');
...
// Ensure pure modal replacement by finding `{selectedOrderDetails && (` all the way to the end of the file.
const modalStartIdx = opFile.indexOf('{selectedOrderDetails && (');
if (modalStartIdx === -1) {
    console.error("No se encontró el inicio del modal en OperadorP2pView.tsx");
    process.exit(1);
}

// Just slice off everything after modalStartIdx and append the cleaned modalFile
// Note: modalFile already includes the closing `    </div>\n  );\n}` from temp_modal_clean.tsx!
let pureModalToAppend = modalFile.substring(modalFile.indexOf('{selectedOrderDetails && ('));

opFile = opFile.substring(0, modalStartIdx) + pureModalToAppend;
