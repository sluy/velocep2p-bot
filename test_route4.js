...
async function fetchBinanceAds(tradeType, amount, bank) {
  const payload = {
    fiat: "VES", page: 1, rows: 20, tradeType, asset: "USDT", countries: [],
  ...
  console.log("API Response:", JSON.stringify(data).substring(0,200)); return (data.code === "000000" && data.data) ? data.data : [];
}
